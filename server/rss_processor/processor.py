...
See `metadata.json` for complete dataset information."""
    ...
    async def list_sources(self) -> List[Dict[str, Any]]:
        """List all available RSS feed sources with their metadata from Hugging Face.
        
        Returns:
            List of dictionaries containing source information with metadata
        """
        ...
        try:
            ...
            try:
                author = self.hf_repo.split('/')[0]
                search_term = self.hf_repo.split('/')[-1]
                self.logger.info(f"Fetching datasets from Hugging Face - Author: {author}, Search: {search_term}")
                
                # Get dataset list from Hugging Face Hub
                datasets_list = list(self.hf_api.list_datasets(
                    author=author,
                    search=search_term
                ))
                self.logger.info(f"Found {len(datasets_list)} datasets from Hugging Face")
                
                for dataset_info in datasets_list:
                    try:
                        self.logger.info(f"Processing dataset: {dataset_info.id}")
                        # Skip the main repo itself, only include datasets within it
                        if dataset_info.id == self.hf_repo:
                            self.logger.debug(f"Skipping main repo: {dataset_info.id}")
                            continue
                            
                        source_info = {
                            "name": dataset_info.id.split('/')[-1],
                            "path": dataset_info.id,
                            "source_url": f"https://huggingface.co/datasets/{dataset_info.id}",
                            "feed_url": "",
                            "feed_title": "",
                            "article_count": 0,
                            "created_at": None,
                            "last_modified": dataset_info.lastModified.isoformat() if hasattr(dataset_info, 'lastModified') else None,
                            "size_mb": getattr(dataset_info, 'size', 0) / (1024 * 1024)  # Convert to MB
                        }
                        
                        # Try to load metadata from JSON file if available
                        try:
                            metadata_url = f"https://huggingface.co/datasets/{dataset_info.id}/raw/main/metadata.json"
                            import requests
                            response = requests.get(metadata_url, timeout=5)
                            if response.status_code == 200:
                                metadata = response.json()
                                source_info.update({
                                    "name": metadata.get('name', source_info['name']),
                                    "source_url": metadata.get('source_url', source_info['source_url']),
                                    "feed_url": metadata.get('feed_url', source_info['feed_url']),
                                    "feed_title": metadata.get('feed_title', source_info['feed_title']),
                                    "article_count": metadata.get('article_count', source_info['article_count']),
                                    "created_at": metadata.get('created_at', source_info['created_at'])
                                })
                                self.logger.debug(f"Loaded metadata for {dataset_info.id}")
                        except Exception as e:
                            self.logger.debug(f"Could not load metadata.json for {dataset_info.id}: {str(e)}")
                        
                        sources.append(source_info)
                        self.logger.info(f"Added dataset: {dataset_info.id}")
                        
                    except Exception as e:
                        self.logger.error(f"Error processing dataset {dataset_info.id}: {str(e)}", exc_info=True)
                        continue
                        
            except Exception as e:
                self.logger.error(f"Error listing datasets from Hugging Face Hub: {str(e)}", exc_info=True)
                # Fallback to local cache if remote listing fails
                self.logger.info("Falling back to local cache due to error")
                return await self._list_local_sources()
            
            # If no sources found from Hugging Face, try local cache
            if not sources:
                return await self._list_local_sources()
            ...
            self.logger.error(f"Error in list_sources: {str(e)}")
            # Fallback to local cache if any error occurs
            return await self._list_local_sources()
